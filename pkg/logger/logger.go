...
func (al *AppLogger) GrpcInfoLogger(gf *GrpcFields) {
	if gf == nil {
		al.SLogger.Error("Print called with nil GrpcFields")
		return
	}

	al.Logger.Info("GRPC",
		zap.String("method", gf.Method),
		zap.Any("request", gf.Request),
		zap.Any("response", gf.Response),
		zap.Duration("duration", gf.Duration),
		zap.Any("metadata", gf.Metadata),
	)
}

func (al *AppLogger) GrpcErrorLogger(gf *GrpcFields) {
	if gf == nil {
		al.SLogger.Error("Print called with nil GrpcFields")
		return
	}

	al.Logger.Error("GRPC",
		zap.String("method", gf.Method),
		zap.Any("request", gf.Request),
		zap.Any("response", gf.Response),
		zap.Duration("duration", gf.Duration),
